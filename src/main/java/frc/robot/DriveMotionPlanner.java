...
import frc.robot.auto.util.cheesypath.lib.physics.DCMotorTransmission;
import frc.robot.auto.util.cheesypath.lib.physics.DifferentialDrive;
import frc.robot.auto.util.cheesypath.lib.trajectory.DistanceView;
import frc.robot.auto.util.cheesypath.lib.trajectory.PurePursuitController;
...
import frc.robot.auto.util.cheesypath.lib.trajectory.TrajectorySamplePoint;
import frc.robot.auto.util.cheesypath.lib.trajectory.TrajectoryUtil;
import frc.robot.auto.util.cheesypath.lib.trajectory.timing.DifferentialDriveDynamicsConstraint;
import frc.robot.auto.util.cheesypath.lib.trajectory.timing.TimedState;
import frc.robot.auto.util.cheesypath.lib.trajectory.timing.TimingConstraint;
import frc.robot.auto.util.cheesypath.lib.trajectory.timing.TimingUtil;
import frc.robot.auto.util.cheesypath.lib.util.CSVWritable;
import frc.robot.auto.util.cheesypath.lib.util.Units;
import frc.robot.auto.util.cheesypath.lib.util.Util;
...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public DriveMotionPlanner() {
        final DCMotorTransmission transmission = new DCMotorTransmission(1.0 / 0.1, 
                                                                         Units.inches_to_meters(3) * Units.inches_to_meters(3) * 60 / (2.0 * 0.01), 
                                                                         1);
        mModel = new DifferentialDrive(60, 10, 15, 
                Units.inches_to_meters(6 / 2.0),
                Units.inches_to_meters(26 / 2.0 * 1),
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        while (actual_lookahead_distance < 24 &&
        ...
        if (actual_lookahead_distance < 24) {
        ...
        DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
        // Feedback on longitudinal error (distance).
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + 4.0 * Units.inches_to_meters
                (mError.getTranslation().x());
